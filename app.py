...
# ================= MODULE 2: COMMUNICATIONS & MISSED CALL AUDIT =================
    ...
    if not st.session_state.missed_calls.empty:
        ...
        if st.button("🤖 Generate AI Call Summary & Action Plan"):
            summary_prompt = "Analyze these missed calls and suggest follow-up actions: " + st.session_state.missed_calls.to_string()
...
# ================= MODULE 3: FINANCIAL LEDGER & EXPENSE INTELLIGENCE =================
    ...
    if not st.session_state.expenses.empty:
        ...
        if st.button("💡 Run AI Expense Optimization Audit"):
            fin_prompt = "Analyze these expenses and provide cost-cutting recommendations: " + st.session_state.expenses.to_string()
    ...
    if st.button("✨ Generate Optimized Copy"):
        if campaign_topic:
            with st.spinner("Crafting engaging social media payload..."):
                copy_prompt = "Create a professional, high-engagement " + platform + " post based on this outline: " + campaign_topic
    ...
    if st.button("📝 Synthesize Executive Meeting Minutes"):
        if audio_input_text:
            with st.spinner("Extracting action items and key decisions..."):
                minutes_prompt = "Extract executive meeting minutes, key decisions, and action items from this transcript: " + audio_input_text
...
# ================= MODULE 10: EXECUTIVE ANALYTICS & KPI CENTER =================
    ...
    if st.button("🚀 Run Comprehensive Enterprise Health Audit"):
        with st.spinner("Executing deep neural audit across all corporate data streams..."):
            ...
            audit_prompt = "Perform a master executive audit across all operational data streams: " + master_summary + ". Provide strategic recommendations."
            audit_report = ask_groq(audit_prompt)
